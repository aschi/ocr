...
public class Character implements Comparable<Character> {
	...
	/*
	 * comparisonVector: Split character matrix into 25 parts (5x5) 0-24: Number
	 * of relevant pixels in each of the 10 parts. scaled to 10 25: Number of
	 * relevant pixels in top half. scaled to 10 26: Number of relevant pixels
	 * in bottom half. scaled to 10 27: width / height ratio scaled to 10 28:
	 * indicator for vertically disjoint character (like i or j) (0 or 1)
	 */
	...
	public Character(ContrastMatrix characterM) {
		...
		if (characterM.getFunctionalChar() != null) {
			...
			for(int i = 0;i < comparisonVector.length;i++){
				comparisonVector[i] = characterM.getFunctionalChar().getCharacter();
			}
		} else {
			...
			if (fh != 0 && fw != 0) {
				...
				// scale parts to 255
				for (int i = 0; i < 25; i++) {
					comparisonVector[i] = (int) Math
							.round(((double) 255 * (double) comparisonVector[i])
									/ ((double) fh * (double) fw));
				}
				...
				comparisonVector[27] = (int) Math
						.round(255 * ((characterM.getHeight() > characterM
								.getWidth()) ? ((double) characterM.getWidth())
								/ ((double) characterM.getHeight())
								: ((double) characterM.getHeight())
	...
	private void setEmphasis() {
		/*
		 * comparisonVector: Split character matrix into 25 parts (5x5) 0-24:
		 * Number of relevant pixels in each of the 10 parts. scaled to 10 25:
		 * Number of relevant pixels in top half. scaled to 10 26: Number of
		 * relevant pixels in bottom half. scaled to 10 27: width / height ratio
		 * scaled to 10 28: indicator for vertically disjoint character (like i
		 * or j) (0 or 1)
		 */
		...
		// 28: should have the highest emphasis, because there can't be f.e. an
		// "a" with a disjointed character
	...
	public boolean equals(Object o) {
		...
		for (int i = 0; i < comparisonVector.length; i++) {
			if (comparisonVector[i] != ((Character) o).getComparisonVector()[i]) {
				diff += Math.abs(comparisonVector[i]
						- ((Character) o).getComparisonVector()[i])
						* emphasisVector[i];
			}
		}
