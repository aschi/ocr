...
public class CharacterComperator {
	...
	public void learn(List<Character> input, List<String> chars, double emphasis) {
		if(input.size() != chars.size()){
			throw new IllegalArgumentException("List dimensions must agree! input: " + input.size() + ", chars: " + chars.size());
		}
		
		for(int i = 0;i < input.size();i++){
			characterRecognitionNetwork.addNeuron(input.get(i), chars.get(i), emphasis);
		}
	}
